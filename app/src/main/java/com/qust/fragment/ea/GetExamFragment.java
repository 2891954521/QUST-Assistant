...
public class GetExamFragment extends BaseEAFragment{
	...
	protected void initLayout(LayoutInflater inflater){
		...
		try{
			exams = (Exam[][]) FileUtils.loadData(new File(activity.getFilesDir(), "exam"));
		}catch(Exception e){
	...
	protected void doQuery() throws NeedLoginException{
		...
		exams[selectTerm] = QUSTQueryModel.queryExam(eaViewModel, y[0], y[1]);
		
		try{
			FileUtils.saveData(new File(activity.getFilesDir(), "exam"), exams);
