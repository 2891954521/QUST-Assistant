...
data class AcademicInfo(
	...
	companion object {
		fun createFromJson(js: JsonObject, entranceTime: Int, group: Int): AcademicInfo {
			val year: Int = if(js.contains("XNM")) {
				js["XNM", ""].toIntOrNull() ?: 0
			} else {
				js["JYXDXNM", ""].toIntOrNull() ?: 0
			}

			val term = if(js.contains("XQMMC")) {
...
data class AcademicGroup(
	...
	class Builder(
		...
		fun build() = AcademicGroup(group, type, requireCredits, obtainedCredits, creditNotEarned, passedCounts, totalCounts)
	}
}